...
def main(): # -----------------------------------------------------------------
    ...
    scraper(rosters_ds,groups_ds,matches_ds)
    ...
    to_csv(rosters_ds, groups_ds, matches_ds) # Save data sets to .csv's
...
def scraper(rosters_ds,groups_ds,matches_ds): # -------------------------------
...
def groups_scraper(matches_soup, year, edition, teams16, teams24, teams32, groups_ds): 
    ...
    if year > 1966 and year not in [1954] or year == 1950:
    ...
    if year <= 1966 and year not in [1930,1950,1954]: # 1966 edition and some prior ones use GR (Goal Ratio) to decide point ties
        col_header = ['Tournament', 'Group','Pos','Team','Pld','W','D','L','GF','GA','GR','Pts','Result']
    else: # Editions after 1966 use GD (Goal Difference) to decide point ties
        col_header = ['Tournament', 'Group','Pos','Team','Pld','W','D','L','GF','GA','GD','Pts','Result']
    cols = [col for col in df.columns if col not in col_header]
    df = df[col_header + cols]
...
def matches_scraper(matches_soup, year, edition, teams16, teams24, teams32, matches_ds): # -------------------
    ...
    for match in match_info:
        ...
        elif year in teams16 and year not in [1958,1974,1978]:
...
def to_csv(rosters_ds, groups_ds, matches_ds):
