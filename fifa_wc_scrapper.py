...
def main(): # -----------------------------------------------------------------
    ...
    years = [y for y in range(1998, 2023, 4) if y not in [1942, 1946]] #[1998]
    ...
    for year, edition in zip(years, wc_editions):
        ...
        groups_scraper(matches_soup, year, edition, groups_ds)
        matches_scraper(matches_soup, year, edition, matches_ds)
...
def roster_scraper(squads_soup, year, edition, rosters_ds): # -----------------
    ...
    if year == 1998: # 1998 edition had a squad size of 22 per team. South Africa presented 23 due to injuries.
    ...
    if year == 1998: # 22 players per team * 32 teams + 1 replacement
        tournament = np.repeat(tournament, 705).tolist()
...
def groups_scraper(matches_soup, year, edition, groups_ds): # ---------------------
    ...
    if year == 1998: groups = groups[14:-5]
...
def matches_scraper(matches_soup, year, edition, matches_ds): # -------------------
